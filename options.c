...
// Bottom line
SetLineG2(&WindowFrame);
setRGB0(&WindowFrame, 0, 0, 255);
setRGB1(&WindowFrame, 0, 0, 100);
...
strcpy ( Move_Entries[5], TXT_Fire[SelectedLanguage] );
...
if (AutoStart == 0 || Pad1.Start == TAP_BUTTON || Pad1.Fire == TAP_BUTTON )
	{
...
//DELETE THIS
	if (Pressed(SELECT_KEY))
		{
		ReadMyTIM((u_long *)FONT8W_TIM);
		InitFont8p(704, 0, 704, 256);

		ReadMyTIM((u_long *)FONT16W_TIM);
		InitFont16p(640, 0, 640, 256);
		}
//----------------------
...
FntPrint(fIdA, "\n\n	PLAY BS MOVIE: [%0d/%0d]", Debug_Config.Debug_BS, TOTAL_BS_MOVIES);
...
if (Pad1.Right == TAP_BUTTON)
	...
	switch (Selected_Option)
		...
		if (Debug_Config.Debug_BS > TOTAL_BS_MOVIES)
...
if (Pad1.Left == TAP_BUTTON)
	...
	switch (Selected_Option)
		...
		Debug_Config.Debug_BS--;
		if (Debug_Config.Debug_BS < 0)
			Debug_Config.Debug_BS = TOTAL_BS_MOVIES;
		BSCurrentFrame = 0;
		break;
...
switch (Verify_Complete)
	...
	CdSearchFile(&VerifyCDFile, "\\DLDEMO.STR;1");
	...
	FntPrint(fIdA, "\n\nSPACE.ACE : %d/%d, %d/%d", Read_Files[1].Position, Expected_Files[1]->Position, Read_Files[1].Size, Expected_Files[1]->Size);
	FntPrint(fIdA, "\n\nSPACE.PAL : %d/%d, %d/%d", Read_Files[2].Position, Expected_Files[2]->Position, Read_Files[2].Size, Expected_Files[2]->Size);
	...
	FntPrint(fIdA, "\n\nDLDEMO.STR : %d/%d, %d/%d", Read_Files[5].Position, Expected_Files[5]->Position, Read_Files[5].Size, Expected_Files[5]->Size);
